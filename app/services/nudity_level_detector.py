...
def detect_nudity_level(messages: Sequence[ChatMessage], check_last: int = 6) -> str:
    ...
    # Определяем уровень
    # КРИТИЧЕСКИ ВАЖНО: "undressing" имеет ВЫСШИЙ приоритет!
    # Если есть слова раздевания - это ВСЕГДА "undressing", даже если есть слова обнажения
    # Это предотвращает переход к "full" когда персонаж только начинает раздеваться
    
    if undressing_count >= 1:
        # Если есть слова раздевания - это ВСЕГДА "undressing"
        # НЕ проверяем другие уровни, если персонаж еще раздевается
        return "undressing"
    elif partial_count >= 1:
        # Частичное обнажение (топлесс) - только если НЕТ слов раздевания
        return "partial"
    elif full_count >= 1:
        # Полное обнажение - только если НЕТ слов раздевания И НЕТ частичного обнажения
        # И только если есть ЯВНОЕ упоминание полного обнажения
        return "full"
    else:
        return "none"
